...
bool delta_has_changes(const domain::DeltaSnapshot& delta) {
    return !delta.added_entries.empty() ||
           !delta.removed_entries.empty() ||
           !delta.modified_entries.empty() ||
           !delta.renamed_entries.empty();
}
...
PipelineResult IndexingPipeline::capture_snapshot_and_delta(const std::filesystem::path& root_path,
    ...
    if (!delta_has_changes(*result.computed_delta)) {
        result.snapshot_build.snapshot = base_snapshot;
        result.computed_delta = domain::DeltaSnapshot {
            domain::kCurrentSchemaHeader,
