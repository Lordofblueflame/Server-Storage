...
StoreStatus LmdbStore::delete_snapshot(const domain::SnapshotId snapshot_id,
                                       SnapshotDeleteStats* out_stats) {
    if (!is_open()) {
        return StoreStatus::failure(StoreErrorCode::NotOpen, "LMDB store is not open.");
    }

    if (snapshot_id == 0U) {
        return StoreStatus::failure(StoreErrorCode::InvalidArgument, "Snapshot id must be non-zero.");
    }
    ...
    auto encoded_snapshot_id = encode_u64_be(snapshot_id);
    MDB_val snapshot_key {};
    snapshot_key.mv_size = encoded_snapshot_id.size();
    snapshot_key.mv_data = encoded_snapshot_id.data();

    const int delete_snapshot_rc = mdb_del(write_txn, snapshots_dbi_, &snapshot_key, nullptr);
    if (delete_snapshot_rc == MDB_NOTFOUND) {
        mdb_txn_abort(write_txn);
        return StoreStatus::failure(StoreErrorCode::NotFound, "Snapshot not found.");
    }
    if (delete_snapshot_rc != MDB_SUCCESS) {
        mdb_txn_abort(write_txn);
        return lmdb_error(delete_snapshot_rc, "mdb_del(delete_snapshot:snapshot)");
    }
    ...
    if (rc != MDB_SUCCESS) {
        mdb_txn_abort(write_txn);
        return lmdb_error(rc, "mdb_cursor_open(delete_snapshot:deltas)");
    }
    ...
    while (rc == MDB_SUCCESS) {
        ...
        if (base_snapshot_id == snapshot_id || target_snapshot_id == snapshot_id) {
            const int delete_delta_rc = mdb_cursor_del(deltas_cursor, 0U);
            if (delete_delta_rc != MDB_SUCCESS) {
                mdb_cursor_close(deltas_cursor);
                mdb_txn_abort(write_txn);
                return lmdb_error(delete_delta_rc, "mdb_cursor_del(delete_snapshot:delta)");
            }
            ++deltas_removed;
    ...
    if (rc != MDB_NOTFOUND) {
        mdb_cursor_close(deltas_cursor);
        mdb_txn_abort(write_txn);
        return lmdb_error(rc, "mdb_cursor_get(delete_snapshot:deltas)");
    }
    mdb_cursor_close(deltas_cursor);

    const int commit_rc = mdb_txn_commit(write_txn);
    if (commit_rc != MDB_SUCCESS) {
        return lmdb_error(commit_rc, "mdb_txn_commit(delete_snapshot)");
    }

    if (out_stats != nullptr) {
        out_stats->snapshots_removed = 1U;
