...
class MockHostIndexerServer final {
public:
    explicit MockHostIndexerServer(const std::uint16_t port)
        : port_(port) {
    }
...
bool run_hostindexer_client_integration_tests() {
    ...
    MockHostIndexerServer server(port);
    server.start();

    asio::io_context io_context;
    localedge::net::HostIndexerClientOptions options;
    options.remote_host = "127.0.0.1";
    options.remote_port = port;
    options.websocket_path = "/hostindexer";
    options.host_id = "localedge-test-host";
    options.client_instance_id = "localedge-test-client";
    ...
    auto client = std::make_shared<localedge::net::HostIndexerWebSocketClient>(io_context, options);
    client->start();
