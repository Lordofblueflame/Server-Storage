...
common::Status parse_header(const std::span<const std::uint8_t> raw, ParsedHeader& out) {
    ...
    if (out.protocol_version != kLocalEdgeProtocolVersion) {
...
std::vector<std::uint8_t> make_frame(const MessageKind kind, const std::span<const std::uint8_t> payload) {
    ...
    common::append_u32_be(out, kLocalEdgeMagic);
    common::append_u16_be(out, kLocalEdgeProtocolVersion);
...
std::vector<std::uint8_t> encode_data(const DataMessage& msg) {
    ...
    common::append_u32_be(payload, static_cast<std::uint32_t>(msg.hostindexer_frame.size()));
    common::append_bytes(payload, msg.hostindexer_frame);
...
common::StatusOr<Message> parse_data_payload(const std::span<const std::uint8_t> payload) {
    ...
    if (!common::read_bytes(payload, offset, frame_len, msg.hostindexer_frame)) {
