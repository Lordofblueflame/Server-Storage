#ifndef LOCALEDGE_PROTOCOL_WIRE_HPP
#define LOCALEDGE_PROTOCOL_WIRE_HPP
...
inline constexpr std::uint32_t kLocalEdgeMagic = 0x4C455347U; // "LESG"
inline constexpr std::uint16_t kLocalEdgeProtocolVersion = 1U;
...
struct ClientHello {
    ...
    std::uint16_t requested_protocol_version {kLocalEdgeProtocolVersion};
...
struct ServerHello {
    bool accepted {false};
    std::uint16_t negotiated_protocol_version {kLocalEdgeProtocolVersion};
...
struct DataMessage {
    ...
    std::vector<std::uint8_t> hostindexer_frame {};
};
...
#endif // LOCALEDGE_PROTOCOL_WIRE_HPP
