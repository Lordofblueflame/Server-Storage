...
const logging::Logger& bridge_logger() {
    static const logging::Logger logger("hugin.bridge");
    return logger;
}
...
void log_bridge_warning(const std::string_view message) {
    bridge_logger().warn("", message);
}
...
std::shared_ptr<MuninnTransportClient> create_muninn_transport_client(
    ...
    switch (options.transport_kind) {
        case MuninnTransportKind::WebSocket:
            return std::make_shared<MuninnWebSocketClient>(
                io_context,
                std::move(options),
                std::move(on_result),
                std::move(on_connection_state)
            );
    }
...
MuninnWebSocketClient::MuninnWebSocketClient(boost::asio::io_context& io_context,
                                                       MuninnClientOptions options,
                                                       CrudResultCallback on_result,
                                                       ConnectionStateCallback on_connection_state)
...
void MuninnWebSocketClient::start() {
    if (running_) {
        return;
    }
    running_ = true;
...
void MuninnWebSocketClient::stop() {
    running_ = false;
    ...
    (void)pull_timer_.cancel();
    (void)reconnect_timer_.cancel();
    resolver_.cancel();
...
bool MuninnWebSocketClient::running() const noexcept {
    return running_;
}

bool MuninnWebSocketClient::connected() const noexcept {
    return running_ && handshake_complete_ && hello_complete_;
}

common::Status MuninnWebSocketClient::submit_command(crud::CrudCommandMessage command) {
    if (!running_) {
        return common::Status::failure(common::ErrorCode::Internal, "Muninn client is not running.");
    }
    if (!connected()) {
        return common::Status::failure(common::ErrorCode::Internal, "Muninn bridge is not connected.");
    }
    ...
    asio::post(io_context_, [self = shared_from_this(), command = std::move(command)]() mutable {
        ...
        if (!self->hello_complete_) {
            log_bridge_warning("dropping command because bridge disconnected before enqueue");
            return;
        }
...
common::Status MuninnWebSocketClient::request_read(const std::uint64_t ack_sequence) {
...
void MuninnWebSocketClient::start_connect() {
    if (!running_) {
        return;
    }
    ...
    resolver_.async_resolve(
        ...
        beast::bind_front_handler(&MuninnWebSocketClient::on_resolve, shared_from_this())
    );
}

void MuninnWebSocketClient::on_resolve(const boost::system::error_code& error,
    ...
    websocket_->next_layer().async_connect(
        results,
        beast::bind_front_handler(&MuninnWebSocketClient::on_connect, shared_from_this())
    );
}

void MuninnWebSocketClient::on_connect(const boost::system::error_code& error,
    ...
    websocket_->set_option(
        websocket::stream_base::decorator([](websocket::request_type& request) {
            request.set(boost::beast::http::field::user_agent, std::string("hugin-gateway"));
        })
    );
    ...
    websocket_->async_handshake(
        host,
        options_.websocket_path,
        beast::bind_front_handler(&MuninnWebSocketClient::on_handshake, shared_from_this())
    );
}

void MuninnWebSocketClient::on_handshake(const boost::system::error_code& error) {
    if (!running_) {
        return;
    }
...
void MuninnWebSocketClient::do_read() {
    ...
    websocket_->async_read(
        read_buffer_,
        beast::bind_front_handler(&MuninnWebSocketClient::on_read, shared_from_this())
    );
}

void MuninnWebSocketClient::on_read(const boost::system::error_code& error, const std::size_t) {
    if (!running_) {
        return;
    }

    if (error == websocket::error::closed) {
        log_bridge_warning("websocket closed by remote");
...
void MuninnWebSocketClient::handle_message(const std::string_view payload) {
    const auto parsed_json = crud::text_to_json(payload);
    if (!parsed_json.has_value() || !parsed_json->is_object()) {
        log_bridge_warning("received malformed json frame from muninn");
        return;
    }
    ...
    if (type == "hello_ack") {
        ...
        if (!hello_ack.has_value() || !hello_ack->accepted) {
            if (hello_ack.has_value()) {
                std::ostringstream text;
                text << "hello rejected by muninn reason=" << hello_ack->reason;
                log_bridge_warning(text.str());
            } else {
                log_bridge_warning("received invalid hello_ack payload from muninn");
            }
    ...
    if (type == "crud_result") {
        ...
        if (!result.has_value()) {
            log_bridge_warning("failed to parse crud_result payload");
            return;
        }
    ...
    {
        std::ostringstream text;
        text << "ignoring unsupported message type=" << type;
        log_bridge_warning(text.str());
    }
}

void MuninnWebSocketClient::send_hello() {
...
void MuninnWebSocketClient::schedule_next_pull(const bool had_records) {
...
void MuninnWebSocketClient::schedule_reconnect(const std::string_view reason) {
    if (!running_) {
        return;
    }
...
std::chrono::milliseconds MuninnWebSocketClient::compute_reconnect_delay() const {
...
void MuninnWebSocketClient::flush_pending_commands() {
...
void MuninnWebSocketClient::notify_connection_state(const bool connected_state, const std::string_view message) {
...
void MuninnWebSocketClient::enqueue_json(const nlohmann::json& payload) {
    enqueue_text(crud::json_to_text(payload));
}

void MuninnWebSocketClient::enqueue_text(std::string payload) {
...
void MuninnWebSocketClient::do_write() {
    ...
    websocket_->async_write(
        ...
        beast::bind_front_handler(&MuninnWebSocketClient::on_write, shared_from_this())
    );
}

void MuninnWebSocketClient::on_write(const boost::system::error_code& error, const std::size_t) {
    if (!running_) {
        return;
    }
...
std::string MuninnWebSocketClient::next_request_id(const std::string_view prefix) {
    const std::uint64_t value = request_sequence_.fetch_add(1U) + 1U;
    return std::string(prefix) + "-" + std::to_string(value);
}

void MuninnWebSocketClient::apply_crud_result(const crud::CrudResultMessage& result) {
    if (on_result_) {
        on_result_(result);
    }
...
void MuninnWebSocketClient::send_ingest_ack(const std::uint64_t ack_sequence, const std::string_view request_id) {
