#ifndef HUGIN_SINKS_MATERIALIZER_SINK_HPP
#define HUGIN_SINKS_MATERIALIZER_SINK_HPP
...
struct HostMaterializedState {
    ...
    std::unordered_map<muninn::domain::EntryId, muninn::domain::FileEntry> entries_by_id {};
    std::unordered_map<std::string, muninn::domain::EntryId> id_by_path {};
};
...
public:
    ...
    common::StatusOr<std::vector<muninn::domain::FileEntry>> list_children(std::string_view host_id,
                                                                                  std::string_view parent_path) const;
    common::StatusOr<std::size_t> entry_count(std::string_view host_id) const;

private:
    static HostMaterializedState build_from_snapshot(const muninn::domain::Snapshot& snapshot,
                                                     std::uint64_t sequence);
...
#endif // HUGIN_SINKS_MATERIALIZER_SINK_HPP
