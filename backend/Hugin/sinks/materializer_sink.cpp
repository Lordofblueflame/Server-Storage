...
void index_entry(HostMaterializedState& state, const muninn::domain::FileEntry& entry) {
    state.id_by_path[entry.normalized_path] = entry.id;
    state.entries_by_id[entry.id] = entry;
}

void remove_entry(HostMaterializedState& state, const muninn::domain::FileEntry& entry) {
...
HostMaterializedState MaterializerSink::build_from_snapshot(const muninn::domain::Snapshot& snapshot,
...
common::Status MaterializerSink::apply_snapshot(const std::string_view host_id,
                                                const std::uint64_t sequence,
                                                const muninn::domain::Snapshot& snapshot) {
...
common::Status MaterializerSink::apply_delta(const std::string_view host_id,
    ...
    for (const auto& renamed : delta.renamed_entries) {
        ...
        muninn::domain::FileEntry renamed_entry = it->second;
...
common::StatusOr<std::string> MaterializerSink::root_path(const std::string_view host_id) const {
    ...
    for (const auto& [entry_id, entry] : state.entries_by_id) {
        (void)entry_id;
        if (entry.type != muninn::domain::EntryType::Directory || entry.normalized_path.empty()) {
            continue;
        }
...
common::StatusOr<std::vector<muninn::domain::FileEntry>> MaterializerSink::list_children(
    const std::string_view host_id,
    const std::string_view parent_path) const {
    if (host_id.empty()) {
        return common::StatusOr<std::vector<muninn::domain::FileEntry>>::failure(
    ...
    if (parent_entry_it->second.type != muninn::domain::EntryType::Directory) {
        return common::StatusOr<std::vector<muninn::domain::FileEntry>>::failure(
            common::ErrorCode::ValidationError,
    ...
    std::vector<muninn::domain::FileEntry> children;
    for (const auto& [entry_id, entry] : state.entries_by_id) {
        (void)entry_id;
    ...
    std::sort(
        ...
        [](const muninn::domain::FileEntry& lhs, const muninn::domain::FileEntry& rhs) {
            const bool lhs_directory = lhs.type == muninn::domain::EntryType::Directory;
            const bool rhs_directory = rhs.type == muninn::domain::EntryType::Directory;
    ...
    return common::StatusOr<std::vector<muninn::domain::FileEntry>>::success(std::move(children));
}
