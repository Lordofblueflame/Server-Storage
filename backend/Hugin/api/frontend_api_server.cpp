...
const logging::Logger& api_logger() {
    static const logging::Logger logger("hugin.api");
    return logger;
}

void log_api(const std::string_view category, const std::string_view message) {
    api_logger().info(category, message);
}

void log_api_warning(const std::string_view category, const std::string_view message) {
    api_logger().warn(category, message);
}

void log_api_error(const std::string_view category, const std::string_view message) {
    api_logger().error(category, message);
}
...
void log_http(const http::request<http::string_body>& request,
    ...
    if (logging::is_http_error_status(result)) {
        log_api_error("http", text.str());
        return;
    }
    if (logging::is_http_warning_status(result)) {
        log_api_warning("http", text.str());
        return;
    }
...
common::StatusOr<nlohmann::json> authorize_request(const http::request<http::string_body>& request,
    ...
    if (!options.local_access_token.empty() && *token == options.local_access_token) {
        return common::StatusOr<nlohmann::json>::success(
            {
                {"sub", "local-web-user"},
                {"auth_type", "local_bearer"}
            }
        );
    }
...
nlohmann::json build_openapi_json(const FrontendApiServerOptions& options) {
    ...
    info["title"] = "Hugin Gateway API";
    info["version"] = "1.0.0";
    info["description"] =
        "Frontend gateway API for forwarding CRUD commands to Muninn and streaming validated results.";
    ...
    void run(http::request<http::string_body> request) {
        ...
        ws_.set_option(
            websocket::stream_base::decorator([](websocket::response_type& response) {
                response.set(http::field::server, std::string("hugin-api"));
            })
        );
        ws_.async_accept(
            request,
        ...
        asio::post(ws_.get_executor(), [self = shared_from_this(), payload = std::move(payload)]() mutable {
            ...
            if (over_limit()) {
                if (options.ws_backpressure_policy == BackpressurePolicy::DropNewest) {
                    log_api_warning("ws", "dropping newest outbound frame due to backpressure");
                    return;
                }
                if (options.ws_backpressure_policy == BackpressurePolicy::DropOldest) {
                    ...
                    if (over_limit()) {
                        log_api_warning("ws", "outbound frame still over limit after dropping oldest, dropping current frame");
                        return;
                    }
                } else {
                    boost::system::error_code ignored;
                    self->ws_.close(websocket::close_code::policy_error, ignored);
                    log_api_warning("ws", "closing websocket due to backpressure policy=disconnect");
                    return;
                }
            }
...
private:
    void do_read() {
        request_ = {};
        parser_.emplace();
        parser_->body_limit(state_->options().max_http_request_body_bytes);
        http::async_read(
            stream_,
            read_buffer_,
            *parser_,
    ...
    void on_read(const beast::error_code& error, const std::size_t) {
        ...
        if (error == http::error::body_limit) {
            parser_.reset();
            log_api_warning("http", "request body exceeded configured parser limit");
            send_json(http::status::payload_too_large, R"({"ok":false,"message":"request body too large"})");
            return;
        }
        if (error) {
            parser_.reset();
        ...
        try {
            ...
            std::ostringstream text;
            text << "unhandled request exception: " << error.what();
            log_api_error("http", text.str());
            send_json(http::status::internal_server_error, R"({"ok":false,"message":"internal_error"})");
        } catch (...) {
            log_api_error("http", "unhandled non-standard request exception");
            send_json(http::status::internal_server_error, R"({"ok":false,"message":"internal_error"})");
        }
    }
    ...
    void handle_http_request(const ParsedTarget& target) {
        ...
        auto parse_json_or_respond = [&]() -> std::optional<nlohmann::json> {
            ...
            if (parsed.ok()) {
                return std::optional<nlohmann::json>(parsed.value);
            }
        ...
        if (request_.method() == http::verb::get && target.path == state_->options().health_path) {
            ...
            body["ok"] = true;
            body["service"] = "hugin-api-gateway";
    ...
    void send_json(const http::status status, std::string body) {
        const auto version = request_.version() == 0U ? 11U : request_.version();
        auto response = std::make_shared<http::response<http::string_body>>(status, version);
        response->set(http::field::server, "hugin-api");
...
common::Status FrontendApiServer::start() {
...

    {
        ...
        text << "listening on " << options_.listen_address << ':' << options_.port
             ...
             << " max_http_request_body_bytes=" << options_.max_http_request_body_bytes
