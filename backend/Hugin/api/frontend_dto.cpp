...
common::StatusOr<FrontendCommandRequestDto> parse_frontend_command_request(const nlohmann::json& json) {
    ...
    auto read_u64 = [&json](const char* field, std::uint64_t& out) -> bool {
        ...
        const auto& value_json = json.at(field);
        if (value_json.is_number_unsigned()) {
            out = value_json.get<std::uint64_t>();
            return true;
        }
        if (!value_json.is_number_integer()) {
            return false;
        }
        const auto value = value_json.get<std::int64_t>();
        if (value < 0) {
            return false;
        }
    ...
    if (json.contains("outbox_batch_size")) {
        const auto& value_json = json.at("outbox_batch_size");
        if (!value_json.is_number_unsigned() && !value_json.is_number_integer()) {
            return common::StatusOr<FrontendCommandRequestDto>::failure(
                common::ErrorCode::ValidationError,
        ...
        if (value_json.is_number_unsigned()) {
            value = value_json.get<std::uint64_t>();
        } else {
            const auto signed_value = value_json.get<std::int64_t>();
            if (signed_value <= 0) {
                return common::StatusOr<FrontendCommandRequestDto>::failure(
                    common::ErrorCode::ValidationError,
